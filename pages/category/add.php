<?php
  ...
  if (isset($_POST["create"])) {
    if (postCategory($_POST) > 0) {
      $isMessage = true;
      $message = "Category \"$_POST[name]\" was successfully added!";
    } else {
      $message = "Category \"$_POST[name]\" failed to add!";
    }
  }
?>
...
<!doctype html>
  <html lang="en">
    <head>
      ...
      <title>Add Category (<?= $_SESSION["role"]; ?>) - Point of Sales</title>
    </head>
        ...
        <form action="" method="post" class="grow px-12 py-6 flex flex-col space-y-6">
          <div class="flex flex-col grow space-y-4">
            <div class="flex justify-between items-center flex-none">
              <span class="text-2xl font-medium">Add Category</span>
            ...
            <div class="space-y-4">
              ...
              <input type="text" id="name" name="name" class="block w-full rounded-full py-2 px-6 border-2 border-slate-200 active:outline-0 text-sky-500" placeholder="Enter a product name" required>
            </div>
          </div>
        </form>
      </div>
