<?php
  ...
  $category = getDatas("SELECT * FROM categories WHERE id = '$_GET[id]'")[0];
  
  $isMessage = false;
  $message = null;
  if (isset($_POST["update"])) {
    if (putCategory($_POST, $category) > 0) {
      $isMessage = true;
      $message = "Category \"$category[name]\" to \"$_POST[name]\" was successfully updated!";
    } else {
      $message = "Category \"$category[name]\" to \"$_POST[name]\" failed to update!";
    }
  }
?>
...
<!doctype html>
  <html lang="en">
    <head>
      ...
      <title>Edit Category (<?= $_SESSION["role"]; ?>) - Point of Sales</title>
    </head>
    
    <body class="flex h-screen fixed w-screen bg-slate-50 text-slate-800">
      ...
      <div class="grow flex flex-col w-0">
        ...
        <form action="" method="post" class="grow px-12 py-6 flex flex-col space-y-6">
          <div class="flex flex-col grow space-y-4">
            <div class="flex justify-between items-center flex-none">
              <span class="text-2xl font-medium">Edit Category</span>
              
              <button type="submit" name="update" class="bg-blue-400 hover:bg-blue-500 flex py-2 px-4 text-white font-medium rounded-full space-x-2">
            ...
            <div class="space-y-4">
              ...
              <input type="text" id="name" name="name" class="block w-full rounded-full py-2 px-6 border-2 border-slate-200 active:outline-0 text-sky-500" placeholder="Enter a product name" required value="<?= $_POST["name"] ?? $category["name"]; ?>">
            </div>
          </div>
        </form>
      </div>
      ...
      <?php if ($isMessage): ?>
        <div id="toast-default" class="fixed right-0 m-4 space-x-4 flex items-center w-full w-max p-4 text-slate-500 bg-slate-50 rounded-2xl shadow-2xl border-2 border-slate-200" role="alert">
          <div class="inline-flex items-center justify-center flex-shrink-0 w-10 h-10 text-sky-500 bg-sky-100 rounded-2xl">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9.568 3H5.25A2.25 2.25 0 003 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 005.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 009.568 3z" />
              <path stroke-linecap="round" stroke-linejoin="round" d="M6 6h.008v.008H6V6z" />
            </svg>
