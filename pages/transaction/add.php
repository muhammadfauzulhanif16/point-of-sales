<?php
  ...
  $rules = getDatas("SELECT * FROM rules WHERE productId = '$product[id]'");
  
  $isMessage = false;
  $message = null;
  if (isset($_POST["create"])) {
    if (postTransaction($_POST, $product) > 0) {
      $isMessage = true;
...
<!doctype html>
  <html lang="en">
    <head>
      ...
      <title>Add Transaction (<?= $_SESSION["role"]; ?>) - Point of Sales</title>
      <style>
    ...
    <body class="flex h-screen fixed w-screen bg-slate-50 text-slate-800 select-none">
        ...
        <form class="grow px-12 py-6 flex flex-col space-y-6" action="" method="post">
          <div class="flex flex-col grow space-y-4">
            <div class="flex justify-between items-center flex-none">
              <span class="text-2xl font-medium">Add Transaction</span>
            ...
            <div class="space-y-6">
              <div class="space-y-4">
                <label for="customerName" class="text-sm font-medium space-y-4"><span>Customer Name</label>
                <input type="text" id="customerName" name="customerName" class="block w-full rounded-full py-2 px-6 border-2 border-slate-200 active:outline-0 text-amber-500" placeholder="Enter a product name">
              </div>
              <div class="space-y-4">
                <label for="productName" class="text-sm font-medium space-y-4"><span>Product Name <span class="text-red-500">*</span></span></label>
                <input type="text" id="productName" name="productName" class="block w-full rounded-full py-2 px-6 border-2 border-slate-200 active:outline-0 text-amber-500" placeholder="Enter the product quantity" required value="<?= $product["name"]; ?>">
              </div>
              <div class="space-y-4">
                <label for="quantity" class="space-y-4 text-sm font-medium">Quantity <span class="text-red-500">*</span> (<?= $product["quantity"] ?> pcs left)</label>
                ...
                <?php if ($product["unitType"] === "Pcs"): ?>
                  <div class="flex space-x-4 items-center w-full rounded-full px-6 border-2 border-slate-200 bg-white">
                    <input type="number" id="quantity" name="quantity" class="w-full px-0 focus:ring-0 border-0 text-amber-500" placeholder="Enter the product quantity" min="1" max="<?= $product["quantity"]; ?>" required>
                    <span class="text-sm font-medium text-slate-500 whitespace-nowrap">| Rp <?= number_format($product["price"], 0, 0, ".") ."/". $product["unitType"]; ?></span>
                  </div>
                <?php else: ?>
                  <select name="quantity" id="quantity" class="block w-full rounded-full py-2 px-6 border-2 border-slate-200 active:outline-0" required>
                    <option disabled selected hidden>Select the product quantity</option>
                    <?php foreach ($rules as $rule): ?>
                      <option value="<?= $rule["id"]; ?>">
                        <?= $rule["title"]; ?> <?= (count(explode(",", $product["unitType"])) > 1) ? explode(",", $product["unitType"])[1] : ""; ?> =
                        <?= $rule["quantity"]; ?> <?= (count(explode(",", $product["unitType"])) > 1) ? explode(",", $product["unitType"])[0] : ""; ?> |
                        Rp<?= number_format($rule["price"], 0, 0, "."); ?>
                      </option>
                    <?php endforeach; ?>
                  </select>
                <?php endif; ?>
      ...
      <!--    <div class="ml-4 text-sm whitespace-nowrap">--><?php //= $message; ?><!--</div>-->
