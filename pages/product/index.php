<?php
  ...
  $products = getDatas("SELECT * FROM products ORDER BY name");
  $categories = getDatas("SELECT * FROM categories ORDER BY name");
  
  if (isset($_GET["search"])) {
    $products = getSearchData($_GET["search"], "products");
  }
?>
...
<!doctype html>
  <html lang="en">
    <head>
      ...
      <title>View Products (<?= $_SESSION["role"]; ?>) - Point of Sales</title>
    </head>
        ...
        <header class="bg-white px-12 py-4 border-b-2 border-slate-200 grid grid-cols-2 flex-none rounded-b-2xl">
          <form action="" class="flex items-center p-0 m-0" method="get">
            ...
            <label for="search" class="w-full">
              <input type="text" id="search" name="search" value="<?= $_GET["search"] ?? "" ?>" placeholder="Search for products by name...." class="w-full border-0 focus:ring-0 text-indigo-500 placeholder-slate-500 p-0">
            </label>
          </form>
        ...
        <div class="grow px-12 py-6 flex flex-col space-y-6">
          <div class="flex flex-col grow space-y-4">
            <div class="flex justify-between items-center flex-none">
              <span class="text-2xl font-medium">View Products</span>
  
              <?php if ($_SESSION["role"] === "admin"): ?>
                <a href="./add.php" class="bg-blue-400 hover:bg-blue-500 flex py-2 px-4 text-white font-medium rounded-full space-x-2">
            ...
            <div class="grow rounded-2xl border-2 border-slate-200 <?= (count($products) === 0) ? "h-full flex flex-col justify-center items-center space-y-4 border-dashed" : "overflow-auto h-0"; ?>">
              <?php if (count($products) === 0): ?>
                ...
                <span class="text-slate-400 font-medium">No products <?= !isset($_GET["search"]) ? "yet" : "found" ?></span>
              <?php else: ?>
                <table class="w-full text-sm text-left text-slate-500">
                  <thead class="text-xs uppercase bg-slate-100 sticky top-0">
                    <tr>
                      ...
                      <th scope="col" class="px-6 py-4 whitespace-nowrap">Unit Type</th>
                  ...
                  <tbody>
                    <?php foreach ($products as $product): ?>
                      <tr class="bg-white border-b hover:bg-slate-200">
                        <th scope="row" class="px-6 py-4 font-medium text-slate-800"><?= $product["name"]; ?></th>
                        <td class="px-6 py-4 whitespace-nowrap"><?= $product["quantity"]; ?> <?= (count(explode(",", $product["unitType"])) > 1) ? explode(",", $product["unitType"])[0] : ""; ?></td>
                        <td class="px-6 py-4 whitespace-nowrap"><?= $product["unitType"]; ?></td>
                        <td class="px-6 py-4 whitespace-nowrap"><?= $product["category"]; ?></td>
                        <td class="px-6 py-4 whitespace-nowrap flex items-center">
                          <?php if ($product["unitType"] === "Pcs"): ?>
                            Rp<?= number_format($product["price"], 0, 0, "."); ?>
                          <?php else: ?>
                            <a data-tooltip-target="tooltip-price<?= $product["id"] ?>" href="./rule.php?id=<?= $product["id"]; ?>" class="inline-flex font-medium text-emerald-500 hover:text-emerald-700">
                              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z" />
                              </svg>
  
  
                              <div id="tooltip-price<?= $product["id"] ?>" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-slate-400 rounded-lg shadow-sm opacity-0 tooltip">
                                View price list of "<?= $product["name"]; ?>"
                                <div class="tooltip-arrow" data-popper-arrow></div>
                              </div>
                            </a>
                          <?php endif; ?>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap"><?= $product["createdAt"]; ?></td>
                        <td class="px-6 py-4 whitespace-nowrap"><?= $product["updatedAt"]; ?></td>
                        
                        <td class="px-6 py-4 whitespace-nowrap flex space-x-4">
                          <?php if ($_SESSION["role"] === "cashier"): ?>
                            <a data-tooltip-target="tooltip-buy-<?= $product["id"]; ?>" href="../transaction/add.php?id=<?= $product["id"]; ?>" class="font-medium text-rose-500 hover:text-rose-700">
                              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
                              </svg>
                              
                              <div id="tooltip-buy-<?= $product["id"] ?>" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-slate-400 rounded-lg shadow-sm opacity-0 tooltip">
                                Buy "<?= $product["name"]; ?>"
                                <div class="tooltip-arrow" data-popper-arrow></div>
                              </div>
                            </a>
                          ...
                          <?php if ($_SESSION["role"] === "admin"): ?>
                            <a data-tooltip-target="tooltip-edit-<?= $product["id"]; ?>" href="./edit.php?id=<?= $product["id"]; ?>" class="font-medium text-blue-500 hover:text-blue-700">
                              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" />
                              </svg>
  
                              <div id="tooltip-edit-<?= $product["id"]; ?>" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-slate-400 rounded-lg shadow-sm opacity-0 tooltip">
                                Edit "<?= $product["name"]; ?>"
                                <div class="tooltip-arrow" data-popper-arrow></div>
                              </div>
                            </a>
                            
                            <a data-tooltip-target="tooltip-delete-<?= $product["id"]; ?>" href="./delete.php?id=<?= $product["id"]; ?>" class="font-medium text-red-500 hover:text-red-700">
                              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                              </svg>
  
                              <div id="tooltip-delete-<?= $product["id"]; ?>" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-slate-400 rounded-lg shadow-sm opacity-0 tooltip">
                                Delete "<?= $product["name"]; ?>"
                                <div class="tooltip-arrow" data-popper-arrow></div>
                              </div>
                            </a>
