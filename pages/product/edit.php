<?php
  ...
  if (isset($_POST["create"])) {
    if (putProduct($_POST, $product) > 0) {
      $isMessage = true;
      $message = "Product \"$product[name]\" to \"$_POST[name]\" was successfully updated!";
    } else {
      $message = "Product \"$product[name]\" to \"$_POST[name]\" failed to update!";
    }
  }
?>
...
<!doctype html>
  <html lang="en">
    <head>
      ...
      <title>Edit Product (<?= $_SESSION["role"]; ?>) - Point of Sales</title>
    </head>
        ...
        <form class="grow px-12 py-6 flex flex-col space-y-6" action="" method="post">
          <div class="flex flex-col grow space-y-4">
            <div class="flex justify-between items-center flex-none">
              <span class="text-2xl font-medium">Edit Product</span>
            ...
            <div class="grid grid-cols-2 gap-6">
              <div class="space-y-4">
                ...
                <input type="text" id="name" name="name" class="block w-full rounded-full py-2 px-6 border-2 border-slate-200 active:outline-0 text-amber-500" placeholder="Enter a product name" required value="<?= $_POST["name"] ?? $product["name"]; ?>">
              </div>
              <div class="space-y-4">
                <label for="quantity" class="text-sm font-medium">Quantity <span class="text-red-500">*</span></label>
                <input type="number" id="quantity" name="quantity" class="block w-full rounded-full py-2 px-6 border-2 border-slate-200 active:outline-0 text-amber-500" placeholder="Enter the product quantity" required value="<?= $_POST["quantity"] ?? $product["quantity"]; ?>">
              </div>
              <div class="space-y-4">
                <label for="unitType" class="text-sm font-medium">Unit Type</label>
                <input type="text" id="unitType" name="unitType" class="block w-full rounded-full py-2 px-6 border-2 border-slate-200 active:outline-0 text-amber-500" placeholder="Enter the product unit type" value="<?= $_POST["unitType"] ?? $product["unitType"]; ?>">
              </div>
              <div class="space-y-4">
...
<!--                            selected="--><?php //($product["category"] === $category["name"]) ? true : false; ?><!--">-->
                      <?= $category["name"]; ?>
                    </option>
                  <?php endforeach; ?>
                </select>
